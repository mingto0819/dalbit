...
export default function Features() {
  return (
    <section className="bg-dalbit-black text-dalbit-silver py-16 md:py-24 border-t border-dalbit-line">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-y-12 md:gap-y-16">
          {features.map(({ Icon, label }) => (
            <div key={label} className="flex flex-col items-center text-center px-4">
              <Icon className="w-14 h-14 md:w-16 md:h-16 text-dalbit-silver holo-glow" />
              <p className="font-display mt-4 md:mt-6 text-sm md:text-base tracking-[0.04em] leading-tight holo-text">
                {label}
              </p>
            </div>
          ))}
